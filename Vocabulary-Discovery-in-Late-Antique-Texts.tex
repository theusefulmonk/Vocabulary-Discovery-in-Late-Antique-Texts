...
\mintinline[]{shell}{pdfgrep}, \mintinline[]{shell}{grep},
\mintinline[]{shell}{awk}, and
\mintinline[]{shell}{lualatex}.\footnote{Other forms of \LaTeX, such as
  \mintinline[]{shell}{pdflatex} and \mintinline[]{shell}{xelatex} will
  serve just as well.} Some information about how to install
\mintinline[]{shell}{pdfgrep} via common package managers and
\mintinline[]{shell}{lualatex} as part of a \TeX distribution is
provided in the acommpanying repository. The tools
\mintinline[]{shell}{grep} and \mintinline[]{shell}{awk} are already
included in any POSIX compliant environment. Any standard shell may be
used. My examples will use \mintinline[]{shell}{zsh}, the default in
MacOS.
...
are used by a tool such as \mintinline[]{shell}{grep} to search one or
more plain text files. The tool \mintinline[]{shell}{pdfgrep} is a free
and open-source variant of \mintinline[]{shell}{grep} that makes it
possible to search a pdf file. Like \mintinline[]{shell}{grep} the
\mintinline[]{shell}{pdfgrep} tool is given a regex and one or more
...
\mintinline[]{shell}{Preview.app} or KDE \mintinline[]{shell}{Okular}.
...
the list of matches produced by \mintinline[]{shell}{pdfgrep} can easily
...
We will use the accompanying file \mintinline[]{shell}{repaginate.tex}.
We start with the scanned pdf's that need repagination in the same
directory as \mintinline[]{shell}{repaginate.tex}. We then make any
necessary changes to the \mintinline[]{shell}{repaginate.tex} file.

If the original file is Beck's translation of the \emph{Hymns on the
Church} with the filename \mintinline[]{shell}{beck_1960.pdf}, then we
...
In \mintinline[]{shell}{repaginate.tex} we comment out lines 38-39
because we don't need any cover pages. Then, we change line 54 to
include pages 1-6, and the filename to
\mintinline[]{shell}{sources/beck_1960.pdf}. We change line 57 to
include the rest of the pages 7-end using the string
\mintinline[]{shell}{7-}. Once again we must update the filename to
\mintinline[]{shell}{sources/beck_1960.pdf}. This will compile a new pdf
...
\mintinline[]{shell}{awk} script without modification. You are free to
...
\mintinline[]{shell}{HdE <whatever else>.pdf} and
\mintinline[]{shell}{HdF <whatever else>.pdf} and
\mintinline[]{shell}{SdF <whatever else>.pdf}

\subsection{Explore}\label{explore}

\subsubsection{Getting a list of
matches}\label{getting-a-list-of-matches}

A basic search that shows how regular expressions can be useful for
capturing text with and without diacriticals.

\begin{minted}[]{sh}
pdfgrep -e '[Ss]ch[aä]tz' HdE\ German.pdf -H --page-number=label
\end{minted}

Using the \mintinline[]{shell}{-H} option forces the filename to be
output when there is only a single text being searched.

\subsubsection{Counting the number of
matches}\label{counting-the-number-of-matches}

\begin{minted}[]{sh}
pdfgrep -e '[Ss]ch[aä]tz' HdE\ German.pdf -H -C
\end{minted}

\subsubsection{Quickly eyeballing the number of matches across different
texts}\label{quickly-eyeballing-the-number-of-matches-across-different-texts}
...
\subsubsection{Dealing with lower quality OCR and spelling
variation}\label{dealing-with-lower-quality-ocr-and-spelling-variation}

\begin{minted}[]{sh}
 pdfgrep *.pdf -e 'G[aei]hen(n)?a' --page-number=label -H
\end{minted}

\subsubsection{Lookbehinds and pipelines when dealing with many false
positives}\label{lookbehinds-and-pipelines-when-dealing-with-many-false-positives}
