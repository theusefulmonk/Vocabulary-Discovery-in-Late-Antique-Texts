...
This presentation introduces a workflow for discovering vocabulary in
digitized late antique texts. I present this workflow first by
identifying the use-cases for it and outlining both the goals this
...
knowledge to use the workflow. I interactively demonstrate its core
steps. I conclude by highlighting the workflow's limitations and
offering suggestions to enable interested attendees to experiment with
...
unicode script. Scans were made for personal use from purchased physical
volumes.
...
\href{https://github.com/theusefulmonk/Vocabulary-Discovery-in-Late-Antique-Texts.git}{public
repository on GitHub} with sample command-line recipes and instructions
allowing any scholar to download and experiment with them.
...
Each of them is conducted at the command line of a UNIX system.
Graphical user interface (GUI) tools have been purposely avoided,
especially since the goal is to offer a cross-platform open source
solution that consumes minimal system resources.
...
the OS. More precisely, the shell is a command interpreter. It provides
a prompt and a command language allowing the user to issue written
commands to the kernel and to orchestrate the operation of many programs
simultaneously. Although there are many different shells available to a
user, most implement some substantial portion of the POSIX standard,
which means that a user who learns one shell's commands can usually
apply that knowledge to any UNIX installation. The tools discussed in
this presentation should be available, if not pre-installed, for any
UNIX system.
...
\TeX{} distribution is provided in the acommpanying repository. The
tools \VERB|\FunctionTok{grep}| and \VERB|\FunctionTok{awk}| are already
...
This workflow turns on two key concepts: pdf page labels and regular
expressions. Such regexes are too complicated to receive a full tutorial
in this presentation, but simple examples in the demonstration portion
will illustrate how they are used. Resources for further study are
available in the associated github repository. In essence, a regex is a
plain text string that represents a pattern. A regex engine evaluates
that string and finds all the strings in the source document that match
the pattern. As originally conceived, regexes are used by a tool such as
\VERB|\FunctionTok{grep}| to search one or more plain text files. The
tool \VERB|\ExtensionTok{pdfgrep}| is a free and open-source variant of
\VERB|\FunctionTok{grep}| that makes it possible to search one or more
pdf files. Like \VERB|\FunctionTok{grep}| the
...
Correspondence to the printed original is what makes
\VERB|\ExtensionTok{pdfgrep}| searches useful. If the labels of the
scanned pdf correspond correctly to the printed original, the list of
matches produced by \VERB|\ExtensionTok{pdfgrep}| can easily be looked
up in either the digital or printed version. Moreover, PDF viewer
software typically provides a keyboard shortcut to jump directly to a
specified page label, a feature that is important when scanned files run
to hundreds of pages and lack other forms of navigable structure.
...
I take for granted that you already have one or more ocr'ed pdf files
...
\LaTeX{} is ``a system for typesetting documents.''\footnote{Leslie
  Lamport, \emph{LATEX: A Document Preparation System : User's Guide and
  Reference Manual} (Boston: Addison-Wesley Professional, 1994), 2.} It
comprises a typesetting language and several typesetting engines for
producing high quality pdfs from a source file. It works a little like
HTML. Just as one marks up plain text to generate a webpage, with
\LaTeX{} one marks up plain text to generate a pdf. \LaTeX{} is
extensible through packages, which typically provide easy ways to to
perform specific complex typesetting tasks. One such package\footnote{\href{https://texdoc.org/serve/pdfpages.pdf/0}{pdfpages}}
can combine source pdfs into a new document. Another\footnote{\href{https://texdoc.org/serve/hyperref/0}{hyperref}}
can edit internal document links and navigable structure, including page
labels. By putting these two packages together, it is possible to create
a source file that will assemble a final pdf from the sections of an
existing pdf, repaginating with correct page labels.

We will use the accompanying file \VERB|\NormalTok{repaginate.tex}|.
Scanned pdfs that need repagination are placed in a dedicated
\texttt{sources} directory. We then make any necessary changes to the
instructions in the \VERB|\NormalTok{repaginate.tex}| file and typeset
it.

In our example, the original file is Beck's translation of the
\emph{Hymns on the Nativity} with the filename
\VERB|\NormalTok{hdn{-}ocr{-}optimized.pdf}|. We first examine the scan
to determine the absolute page numbers of each section requiring
distinct pagination:
...
1-14, and the filename to
\VERB|\NormalTok{sources/hdn{-}ocr{-}optimized.pdf}|. We change line 57
to include the rest of the pages 15-end using the string
\VERB|\NormalTok{15{-}}|. Once again we must update the filename to
\VERB|\NormalTok{sources/hdn{-}ocr{-}optimized}|. This will compile a
new pdf with the specified pages, using the specified page labels.
...
\tightlist
\item
  \textbf{Repagination via lualatex}
\end{enumerate}
...
The result should be a pdf file with the correct page labels.(For the
purposes the demonstration, I will move the file to the \texttt{corpus}
directory:
\VERB|\FunctionTok{mv}\NormalTok{ repaginate.pdf ./corpus/HdN\textbackslash{} German.pdf}|)
...
establish a file naming convention that makes it easy to produce a
useful report at the end. The convention is convenient, but arbitrary,
and is necessary only if you want to use my \VERB|\FunctionTok{awk}|
script without modification. You are free to re-write it to follow some
other convention. The included \VERB|\FunctionTok{awk}| script is
...
The result shows that although the \emph{HdV} and the \emph{HdE} are
comparable in line count, \emph{HdE} seems to use the language of
treasure more frequently.
...
we must use perl compatible regular expressions, a more sophisticated
tool. We invoke them via the \texttt{-P} flag. The negative lookbehind
itself is \texttt{(?\textless{}!\ .\ .\ .)} and the negative lookahead
is \texttt{(?!\ .\ .\ .)}. This excludes matches in which the word
\emph{Ende} is preceded or followed by the most common patterns we wish
to exclude.

A problem remains, however. OCR'ed texts often have inconsistent
...
the screen, referred to as standard output: \texttt{stdout}. Once you
have built up a number of small searches that can easily be recalled
...
Lamport, Leslie. \emph{LATEX: A Document Preparation System : User's
Guide and Reference Manual}. Boston: Addison-Wesley Professional, 1994.
