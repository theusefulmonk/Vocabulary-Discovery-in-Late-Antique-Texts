...
\mintinline[]{bash}{pdfgrep}, \mintinline[]{bash}{grep},
\mintinline[]{bash}{awk}, and \mintinline[]{bash}{lualatex}.\footnote{Other
  forms of \LaTeX, such as \mintinline[]{bash}{pdflatex} and
  \mintinline[]{bash}{xelatex} will serve just as well.} Some
information about how to install \mintinline[]{bash}{pdfgrep} via common
package managers and \mintinline[]{bash}{lualatex} as part of a
\TeX distribution is provided in the acommpanying repository. The tools
\mintinline[]{bash}{grep} and \mintinline[]{bash}{awk} are already
included in any POSIX compliant environment. Any standard shell may be
used. My examples will use \mintinline[]{bash}{zsh}, the default in
MacOS.
...
are used by a tool such as \mintinline[]{bash}{grep} to search one or
more plain text files. The tool \mintinline[]{bash}{pdfgrep} is a free
and open-source variant of \mintinline[]{bash}{grep} that makes it
possible to search a pdf file. Like \mintinline[]{bash}{grep} the
\mintinline[]{bash}{pdfgrep} tool is given a regex and one or more
...
\mintinline[]{bash}{Preview.app} or KDE \mintinline[]{bash}{Okular}.
...
the list of matches produced by \mintinline[]{bash}{pdfgrep} can easily
...
We will use the accompanying file \mintinline[]{bash}{repaginate.tex}.
We start with the scanned pdf's that need repagination in the same
directory as \mintinline[]{bash}{repaginate.tex}. We then make any
necessary changes to the \mintinline[]{bash}{repaginate.tex} file.

If the original file is Beck's translation of the \emph{Hymns on the
Church} with the filename \mintinline[]{bash}{beck_1960.pdf}, then we
...
In \mintinline[]{bash}{repaginate.tex} we comment out lines 38-39
because we don't need any cover pages. Then, we change line 54 to
include pages 1-6, and the filename to
\mintinline[]{bash}{sources/beck_1960.pdf}. We change line 57 to include
the rest of the pages 7-end using the string \mintinline[]{bash}{7-}.
Once again we must update the filename to
\mintinline[]{bash}{sources/beck_1960.pdf}. This will compile a new pdf
...
\mintinline[]{bash}{awk} script without modification. You are free to
...
filenames for each would be: \mintinline[]{bash}{HdE <whatever>.pdf} and
\mintinline[]{bash}{HdF <whatever>.pdf} and
\mintinline[]{bash}{SdF <whatever>.pdf}
...
Using the \mintinline[]{bash}{-H} option forces the filename to be
